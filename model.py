...
# Residual Block Architecture - based on ResNet paper
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # stride will change the input <> output matrix sizes making them incompatible
    ...
    def forward(self, x, fmap_dict=None, prefix=""):
        ...
        out_add = out + shortcut  # Add input to residual block to prevent gradient vanishing
...
class AudioCNN(nn.Module):
    def __init__(self, num_classes=50):
        super().__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(1, 64, 7, stride=2, padding=3, bias=False), nn.BatchNorm2d(
    ...
    def forward(self, x, return_feature_maps=False):
        if not return_feature_maps:
            ...
            x = self.avgpool(x)
            x = x.view(x.size(0), -1)  # reshape tensor without chaging data
            x = self.dropout(x)
            x = self.fc(x)

            return x  # classification result
            ...
            return x, feature_maps  # classification result and feature maps
